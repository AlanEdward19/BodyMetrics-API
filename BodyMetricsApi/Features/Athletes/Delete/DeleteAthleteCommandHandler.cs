...
public sealed class DeleteAthleteCommandHandler(IAthleteRepository athleteRepository)
{
    public async Task<OperationResult> HandleAsync(DeleteAthleteCommand command, CancellationToken cancellationToken)
    {
        var deleted = await athleteRepository.DeleteAsync(command.Id, cancellationToken);
        return deleted
